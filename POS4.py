...
class PharmacyPOS:
        ...
        try:
            with self.conn:
                ...
                CREATE TABLE IF NOT EXISTS inventory (
                    ...
                    unit_price REAL,  -- Cost price
                    quantity INTEGER,
                    supplier TEXT
                )
            ''')
            # Check if unit_price and supplier columns exist and add them if not
    ...
    def initialize_inventory_with_receipt(self):
        sample_items = [
            ("MED001", "Pain Reliever", "Medicine", 10.00, 7.00, 100, "PharmaCorp"),  # Retail: 10, Unit: 7
            ("SUP001", "Vitamin C", "Supplement", 5.00, 3.50, 200, "HealthSupplies Inc"),  # Retail: 5, Unit: 3.5
            ("DEV001", "Thermometer", "Medical Device", 15.00, 10.00, 50, "MediTech Ltd"),  # Retail: 15, Unit: 10
        ]
        with self.conn:
            cursor = self.conn.cursor()
            for item_id, name, item_type, price, unit_price, quantity, supplier in sample_items:
                cursor.execute("INSERT OR IGNORE INTO inventory (item_id, name, type, price, unit_price, quantity, supplier) VALUES (?, ?, ?, ?, ?, ?, ?)",
                            (item_id, name, item_type, price, unit_price, quantity, supplier))
            self.conn.commit()
    ...
    def update_cart_table(self) -> None:
        ...
        # Populate cart table with current cart items
        for item in self.cart:
            self.cart_table.insert("", "end", values=(
                item["name"], f"{item['price']:.2f}", item["quantity"], f"{item['subtotal']:.2f}"
            ))
        # Update totals and quantity display
        self.update_cart_totals()
        self.update_quantity_display()
        # Force a UI refresh to ensure display
        self.root.update_idletasks()

    def force_ui_refresh(self) -> None:
    # Force Tkinter to redraw the UI
        self.root.update()
        self.root.update_idletasks()
    ...
    def show_add_item(self) -> None:
        ...
        fields = ["Item ID (Barcode)", "Product Name", "Price", "Unit Price", "Quantity", "Supplier"]
        entries = {}
        for field in fields:
    ...
    def add_item(self, item_id: str, name: str, item_type: str, price: str, unit_price: str, quantity: str, supplier: str, window: tk.Toplevel) -> None:
        try:
            price = float(price)
            ...
            if not all([name, item_type]):
                ...
                return
            if price < unit_price:
                messagebox.showerror("Error", "Retail price cannot be less than unit price", parent=self.root)
                return
            ...
            messagebox.showerror("Error", "Invalid price, unit price, or quantity", parent=self.root)
    ...
    def show_update_item(self, item: tuple) -> None:
        ...
        field_indices = {
            ...
            "Product Name": 1,
            "Price": 3,
            "Unit Price": 4,  # Adjusted index for unit_price
            "Quantity": 5,
            "Supplier": 6
        }
        for field in fields:
    ...
    def update_item(self, item_id: str, name: str, item_type: str, price: str, unit_price: str, quantity: str, supplier: str, original_item_id: str, window: tk.Toplevel) -> None:
        try:
            price = float(price)
            unit_price = float(unit_price)
            quantity = int(quantity)
            if price < 0 or unit_price < 0 or quantity < 0:
                messagebox.showerror("Error", "Price, unit price, and quantity cannot be negative", parent=self.root)
                return
            if price < unit_price:
                messagebox.showerror("Error", "Retail price cannot be less than unit price", parent=self.root)
                return
            ...
            with self.conn:
                ...
                cursor.execute("""
                    UPDATE inventory
                    SET item_id = ?, name = ?, type = ?, price = ?, unit_price = ?, quantity = ?, supplier = ?
                    WHERE item_id = ?
                """, (item_id, name, item_type, price, unit_price, quantity, supplier, original_item_id))
                cursor.execute("INSERT INTO transaction_log (log_id, action, details, timestamp, user) VALUES (?, ?, ?, ?, ?)",
                            (str(uuid.uuid4()), "Update Item", f"Updated item {item_id}: {name}, {quantity} units, Price: {price:.2f}, Unit Price: {unit_price:.2f}, Supplier: {supplier}",
                            datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.current_user))
                self.conn.commit()
                self.update_inventory_table()
                window.destroy()
    ...
    def show_sales_summary(self) -> None:
        ...
        for col, head in zip(columns, headers):
            inventory_table.heading(col, text=head)
            inventory_table.column(col, width=150 if col != "Items" else 300, anchor="center" if col != "Items" else "w")
        inventory_table.pack(fill="both", expand=True, pady=10)

        def update_sales_table(event: Optional[tk.Event] = None) -> None:
            ...
            try:
                with self.conn:
                    cursor = self.conn.cursor()
                    sql = ("SELECT transaction_id, items, total_amount, timestamp FROM transactions "
                        "WHERE transaction_id LIKE ? AND status = 'Completed'")
                    cursor.execute(sql, (f"%{query}%",) if query else ())
                    for transaction in cursor.fetchall():
                        transaction_id, items_str, total_amount, timestamp = transaction
                        # Handle None values
                        ...
                        inventory_table.insert("", "end", values=(
                            transaction_id,
                            items_display,
                            f"{total_amount:.2f}",
                            timestamp
                ...
                messagebox.showerror("Error", f"Failed to load sales data: {e}", parent=self.root)
            # Force UI refresh
            self.root.update_idletasks()

        # Bind search and initial table update
        search_entry.bind("<KeyRelease>", update_sales_table)
        update_sales_table()
        ...
        try:
            with self.conn:
                        ...
                        try:
                            ...
                            if item:
                                ...
                                if qty > available_qty:
                                    messagebox.showwarning("Warning", f"Insufficient stock for {item[1]}: requested {qty}, available {available_qty}", parent=window)
                                    qty = available_qty
                                if qty > 0:
                                    self.cart.append({
                                        "id": item[0],
                            ...
                            messagebox.showwarning("Warning", f"Invalid item data format: {item_data} ({e})", parent=window)
                # Set customer ID and update label
                ...
                cursor.execute("INSERT INTO transaction_log (log_id, action, details, timestamp, user) VALUES (?, ?, ?, ?, ?)",
                            ...
                            datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.current_user))
                self.conn.commit()
            # Force update cart table and UI
            self.force_update_cart_table()
            self.root.update_idletasks()  # Process pending UI tasks
            window.destroy()
            # Switch to dashboard and force UI refresh
            self.show_dashboard()
            self.force_ui_refresh()
            # Debug cart contents
            print("Cart after resuming:", [f"{item['name']} (x{item['quantity']})" for item in self.cart])
