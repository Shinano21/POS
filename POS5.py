...
class PharmacyPOS:
        ...
        try:
            ...
            with self.conn:
                ...
                if new_quantity > inventory_qty:
                    ...
                    self.update_quantity_display()
                    return
                # Update cart only, don't touch inventory here
    ...
    def show_sales_summary(self) -> None:
        ...
        with self.conn:
            ...
            # Monthly sales, excluding returned transactions
            ...
            # Daily sales, excluding returned transactions
    ...
    def show_add_user(self) -> None:
        ...
        ttk.Combobox(add_box, textvariable=role_var, values=["User", "Drug Lord"], state="readonly",
                     font=("Helvetica", 14)).pack(pady=5)

        tk.Button(add_box, text="Add User",
                  command=lambda: self.add_user(entries["Username"].get(), entries["Password"].get(), role_var.get(), window),
                  bg="#2ecc71", fg="#ffffff", font=("Helvetica", 14), activebackground="#27ae60", activeforeground="#ffffff",
                  padx=12, pady=8, bd=0).pack(pady=15)
    ...
    def show_update_user(self) -> None:
        ...
        with self.conn:
            ...
            if user:
                ...
                tk.Label(update_box, text="Update User", font=("Helvetica", 18, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=15)
                ...
                ttk.Combobox(update_box, textvariable=role_var, values=["User", "Drug Lord"], state="readonly",
                             font=("Helvetica", 14)).pack(pady=5)
                ...
                tk.Button(update_box, text="Update User",
                          ...
                          bg="#2ecc71", fg="#ffffff", font=("Helvetica", 14), activebackground="#27ae60",
                          activeforeground="#ffffff", padx=12, pady=8, bd=0).pack(pady=15)
    ...
    def show_customer_management(self) -> None:
        ...
        tk.Label(content_frame, text="Customer Management", font=("Helvetica", 18, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)

        search_frame = tk.Frame(content_frame, bg="#ffffff")
        search_frame.pack(fill="x", pady=10)
        tk.Label(search_frame, text="Search by Name:", font=("Helvetica", 14), bg="#ffffff", fg="#1a1a1a").pack(side="left")
        ...
        tk.Button(search_frame, text="Add New Customer", command=self.show_add_customer,
                  bg="#2ecc71", fg="#ffffff", font=("Helvetica", 14), activebackground="#27ae60",
                  activeforeground="#ffffff", padx=12, pady=8, bd=0).pack(side="right", padx=5)

        customer_frame = tk.Frame(content_frame, bg="#ffffff", bd=1, relief="flat")
        ...
        self.customer_table = ttk.Treeview(customer_frame, columns=columns, show="headings")
        for col, head in zip(columns, headers):
        ...
        button_frame = tk.Frame(content_frame, bg="#ffffff")
        button_frame.pack(fill="x", pady=10)
        self.update_customer_btn = tk.Button(button_frame, text="Update Customer", command=self.show_update_customer,
                                            bg="#2ecc71", fg="#ffffff", font=("Helvetica", 14),
                                            activebackground="#27ae60", activeforeground="#ffffff",
                                            padx=12, pady=8, bd=0, state="disabled")
    ...
    def update_customer_table(self, event: Optional[tk.Event] = None) -> None:
        ...
        with self.conn:
            ...
            for customer in cursor.fetchall():
                self.customer_table.insert("", "end", values=(customer[0], customer[1], customer[2] or "N/A", customer[3] or "N/A"))
    ...
    def show_add_customer(self) -> None:
        ...
        tk.Label(content_frame, text="Add New Customer", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)

        fields = ["Name", "Contact", "Address"]
        entries = {}
        for field in fields:
            tk.Label(content_frame, text=field, font=("Helvetica", 12), bg="#ffffff", fg="#1a1a1a").pack(anchor="w")
            entry = tk.Entry(content_frame, font=("Helvetica", 12), bg="#f5f6f5")
            entry.pack(fill="x", pady=5)
            entries[field] = entry

        tk.Button(content_frame, text="Save Customer",
                  command=lambda: self.add_customer(entries["Name"].get(), entries["Contact"].get(), entries["Address"].get(), window),
                  bg="#2ecc71", fg="#ffffff", font=("Helvetica", 12), activebackground="#27ae60", activeforeground="#ffffff",
    ...
    def show_update_customer(self) -> None:
        ...
        with self.conn:
            ...
            if customer:
                ...
                tk.Label(content_frame, text="Update Customer", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)
                ...
                for i, field in enumerate(fields):
                    ...
                    entry.pack(fill="x", pady=5)
                    entries[field] = entry
                    entry.insert(0, customer[i + 1] or "")

                tk.Button(content_frame, text="Update Customer",
                          command=lambda: self.update_customer(customer[0], entries["Name"].get(), entries["Contact"].get(),
                                                              entries["Address"].get(), window),
                          bg="#2ecc71", fg="#ffffff", font=("Helvetica", 12), activebackground="#27ae60",
                          activeforeground="#ffffff", padx=12, pady=8, bd=0).pack(pady=10)
    ...
    def select_customer(self, event: Optional[tk.Event] = None) -> None:
        ...
        tk.Label(content_frame, text="Select Customer", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)

        search_frame = tk.Frame(content_frame, bg="#ffffff")
        search_frame.pack(fill="x", pady=5)
        tk.Label(search_frame, text="Search by Name:", font=("Helvetica", 12), bg="#ffffff", fg="#1a1a1a").pack(side="left")
        search_entry = tk.Entry(search_frame, font=("Helvetica", 12), bg="#f5f6f5")
        ...
        columns = ("CustomerID", "Name", "Contact", "Address")
        headers = ("CUSTOMER ID", "NAME", "CONTACT", "ADDRESS")
        customer_list = ttk.Treeview(customer_frame, columns=columns, show="headings")
        for col, head in zip(columns, headers):
            customer_list.heading(col, text=head)
            customer_list.column(col, width=150 if col != "Name" else 200, anchor="center" if col != "Name" else "w")
        customer_list.pack(fill="both", expand=True)

        def update_customer_list(event=None):
            ...
            with self.conn:
                ...
                cursor.execute(sql, (f"%{query}%",) if query else ())
                for customer in cursor.fetchall():
                    customer_list.insert("", "end", values=(customer[0], customer[1], customer[2] or "N/A", customer[3] or "N/A"))

        search_entry.bind("<KeyRelease>", update_customer_list)
        update_customer_list()

        def select_customer_action():
            selected_item = customer_list.selection()
            if selected_item:
                customer_id, customer_name = customer_list.item(selected_item)["values"][:2]
                self.current_customer_id = customer_id
                self.customer_id_label.config(text=customer_name)
                window.destroy()

        tk.Button(content_frame, text="Select", command=select_customer_action,
                  bg="#2ecc71", fg="#ffffff", font=("Helvetica", 12), activebackground="#27ae60",
                  activeforeground="#ffffff", padx=12, pady=8, bd=0).pack(pady=10)
        customer_list.bind("<Double-1>", lambda e: select_customer_action())
    ...
    def opening_closing_fund(self, event: Optional[tk.Event] = None) -> None:
        ...
        tk.Label(content_frame, text="Opening/Closing Fund", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)

        tk.Label(content_frame, text="Amount", font=("Helvetica", 12), bg="#ffffff", fg="#1a1a1a").pack(anchor="w")
        amount_entry = tk.Entry(content_frame, font=("Helvetica", 12), bg="#f5f6f5")
        amount_entry.pack(fill="x", pady=5)

        type_var = tk.StringVar(value="Opening")
        tk.Label(content_frame, text="Type", font=("Helvetica", 12), bg="#ffffff", fg="#1a1a1a").pack(anchor="w")
        ttk.Combobox(content_frame, textvariable=type_var, values=["Opening", "Closing"], state="readonly",
                     font=("Helvetica", 12)).pack(pady=5)

        tk.Button(content_frame, text="Submit",
                  command=lambda: self.create_password_auth_window(
                      "Authenticate Fund", "Enter admin password to record fund",
                      self.validate_fund_auth, amount=amount_entry.get(), fund_type=type_var.get(), window=window),
                  bg="#2ecc71", fg="#ffffff", font=("Helvetica", 12), activebackground="#27ae60",
                  activeforeground="#ffffff", padx=12, pady=8, bd=0).pack(pady=10)

    def validate_fund_auth(self, password: str, window: tk.Toplevel, **kwargs) -> None:
        amount = kwargs.get("amount")
        fund_type = kwargs.get("fund_type")
        fund_window = kwargs.get("window")
        with self.conn:
            cursor = self.conn.cursor()
            cursor.execute("SELECT password FROM users WHERE role = 'Drug Lord' LIMIT 1")
            admin_password = cursor.fetchone()
            if admin_password and password == admin_password[0]:
                try:
                    ...
                    window.destroy()
                    messagebox.showerror("Error", f"Invalid amount: {e}", parent=self.root)
            else:
                window.destroy()
                messagebox.showerror("Error", "Invalid admin password", parent=self.root)
    ...
    def void_selected_items(self, event: Optional[tk.Event] = None) -> None:
        if not self.cart or self.selected_item_index is None:
            ...
            return
        self.create_password_auth_window("Authenticate Void Item", "Enter admin password to void selected item",
        ...
        try:
            ...
            with self.conn:
                ...
                cursor.execute("INSERT INTO transactions (transaction_id, items, total_amount, cash_paid, change_amount, timestamp, status, payment_method, customer_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               (transaction_id, items, total_amount, 0.0, 0.0, timestamp, "Held", None, None))
                self.conn.commit()
            self.cart.clear()
            self.selected_item_index = None
            self.discount_var.set(False)
            self.discount_authenticated = False
            self.update_cart_table()
            messagebox.showinfo("Success", "Transaction held successfully", parent=self.root)
        except sqlite3.Error as e:
    ...
    def view_unpaid_transactions(self, event: Optional[tk.Event] = None) -> None:
        ...
        tk.Label(content_frame, text="Unpaid Transactions", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)
        ...
        for col, head in zip(columns, headers):
            ...
            unpaid_table.column(col, width=200 if col == "Items" else 150, anchor="center" if col != "Items" else "w")
                ...
                for item_data in items.split(";"):
                    if item_data:
                        ...
                        if item and item[3] >= int(qty):
                            ...
                            messagebox.showerror("Error", f"Insufficient stock for {item[1]}", parent=window)
                            return
        ...
        tk.Button(content_frame, text="Resume Transaction", command=resume_transaction,
                  bg="#2ecc71", fg="#ffffff", font=("Helvetica", 12), activebackground="#27ae60",
                  activeforeground="#ffffff", padx=12, pady=8, bd=0).pack(pady=10)
        unpaid_table.bind("<Double-1>", lambda e: resume_transaction())
    ...
    def mode_of_payment(self, event: Optional[tk.Event] = None) -> None:
        ...
        tk.Label(content_frame, text="Select Payment Method", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)

        payment_var = tk.StringVar(value="Cash")
        payment_methods = ["Cash", "Card", "Mobile Payment"]
        for method in payment_methods:
            tk.Radiobutton(content_frame, text=method, value=method, variable=payment_var,
                           font=("Helvetica", 12), bg="#ffffff", fg="#1a1a1a").pack(anchor="w", pady=5)

        tk.Button(content_frame, text="Confirm",
    ...
    def return_transaction(self, event: Optional[tk.Event] = None) -> None:
        self.create_password_auth_window("Authenticate Return", "Enter admin password to initiate return",
    ...
    def show_return_transaction(self, transaction_id: Optional[str] = None) -> None:
        ...
        tk.Label(content_frame, text="Return Transaction", font=("Helvetica", 16, "bold"), bg="#ffffff", fg="#1a1a1a").pack(pady=10)

        search_frame = tk.Frame(content_frame, bg="#ffffff")
        search_frame.pack(fill="x", pady=5)
        tk.Label(search_frame, text="Search by Transaction ID:", font=("Helvetica", 12), bg="#ffffff", fg="#1a1a1a").pack(side="left")
        search_entry = tk.Entry(search_frame, font=("Helvetica", 12), bg="#f5f6f5")
        search_entry.pack(side="left", fill="x", expand=True, padx=5)

        return_frame = tk.Frame(content_frame, bg="#ffffff", bd=1, relief="flat")
        return_frame.pack(fill="both", expand=True, pady=10)

        columns = ("TransactionID", "Items", "TotalAmount", "Timestamp", "Status")
        headers = ("TRANSACTION ID", "ITEMS", "TOTAL AMOUNT", "TIMESTAMP", "STATUS")
        self.return_table = ttk.Treeview(return_frame, columns=columns, show="headings")
        for col, head in zip(columns, headers):
            self.return_table.heading(col, text=head)
            self.return_table.column(col, width=200 if col == "Items" else 150, anchor="center" if col != "Items" else "w")
        self.return_table.pack(fill="both", expand=True)

        def update_return_table(event=None):
            for item in self.return_table.get_children():
                self.return_table.delete(item)
            with self.conn:
                cursor = self.conn.cursor()
                query = search_entry.get().strip()
                if query:
                    ...
                    for item_data in items_str.split(";"):
                        if item_data:
                            ...
                            if name:
                                item_names.append(f"{name[0]} (x{qty})")
                    items_display = ", ".join(item_names)[:100] + "..." if len(", ".join(item_names)) > 100 else ", ".join(item_names) if item_names else "No items"
                    self.return_table.insert("", "end", values=(transaction[0], items_display, f"{transaction[2]:.2f}", transaction[3], transaction[4]))

        search_entry.bind("<KeyRelease>", update_return_table)
        update_return_table()

        button_frame = tk.Frame(content_frame, bg="#ffffff")
        button_frame.pack(fill="x", pady=10)
        self.process_transaction_btn = tk.Button(button_frame, text="Process Transaction", command=self.process_return,
                                                bg="#2ecc71", fg="#ffffff", font=("Helvetica", 12),
                                                activebackground="#27ae60", activeforeground="#ffffff",
                                                padx=12, pady=8, bd=0, state="disabled")
        self.process_transaction_btn.pack(pady=10)
